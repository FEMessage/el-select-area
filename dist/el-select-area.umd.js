(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@babel/runtime/helpers/slicedToArray'), require('lodash.memoize')) :
  typeof define === 'function' && define.amd ? define(['exports', '@babel/runtime/helpers/slicedToArray', 'lodash.memoize'], factory) :
  (global = global || self, factory(global.Component = {}, global._slicedToArray, global.memoize));
}(this, function (exports, _slicedToArray, memoize) { 'use strict';
  ...
  function isBelongToProvince() {
    var code1 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    var code2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
    return isCode(code1) && isCode(code2) && code1.slice(0, 2) === code2.slice(0, 2);
  }

  function isBelongToCity() {
    var code1 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    var code2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
    return isCode(code1) && isCode(code2) && code1.slice(0, 4) === code2.slice(0, 4);
  }
  ...
  var script = {
    ...
    props: {
      ...
      size: {
        ...
        "default": 'medium',
        validator: function validator(val) {
          return ['small', 'medium', 'mini'].indexOf(val) > -1;
        }
      },
    ...
    data: function data() {
      return {
        ...
        getList: memoize(this.getAreaList, function () {
          return (arguments.length <= 1 ? undefined : arguments[1]) ? arguments.length <= 1 ? undefined : arguments[1] : arguments.length <= 0 ? undefined : arguments[0];
        }),
        getIndex: memoize(this.getAreaIndex, function () {
    ...
    methods: {
       ...
       */
      setArea: function setArea(type) {
        var _this = this;

        var columnNum = type === 'province' ? 0 : type === 'city' ? 1 : 2;
        return function () {
          ...
          if (!code) return; // 传入默认code事补全中文

          if (!name) {
            name = _this[type][code];
          }

          _this.$set(_this.values, columnNum, {
            code: code,
            name: name,
            type: type
          });
          ...
          if (index !== null) {
            _this.$set(_this.indexs, columnNum, index);
          }
        };
      },
      provinceChange: function provinceChange(item, index) {
        var code = item.code,
            name = item.name;
      ...
      cityChange: function cityChange(item, index) {
        var code = item.code,
            name = item.name;
        this.setCity(item, index);

        if (this.level >= 2) {
          var county = this.getList('county', code.slice(0, 4));
          this.setList('county', code);
          this.countyChange(county[0], 0);
        }
      },
      countyChange: function countyChange(item, index) {
        var code = item.code,
            name = item.name;
        this.setCounty(item, index);
      },
      handleOptionChange: function handleOptionChange(index, type) {
        var _this$displayColumns = _slicedToArray(this.displayColumns, 3),
            province = _this$displayColumns[0],
            city = _this$displayColumns[1],
            county = _this$displayColumns[2];
       ...
       * @returns {Array}
       */
      getAreaList: function getAreaList(type, code) {
        var result = []; // 最高级行政区域可以不需要传入code
      ...
      setValues: function setValues() {
        // 默认省级区域
        this.setList('province'); // 设置传入的选中值

        var _this$value = _slicedToArray(this.value, 3),
            provinceCode = _this$value[0],
            cityCode = _this$value[1],
            countyCode = _this$value[2];

        var provinceIndex = provinceCode ? this.getIndex('province', provinceCode) : -1;
        var cityIndex = cityCode ? this.getIndex('city', cityCode) : -1;
        var countyIndex = countyCode ? this.getIndex('county', countyCode) : -1;
        ...
        if (isBelongToProvince(provinceCode, cityCode) && this.level >= 1 && cityIndex > -1) {
          this.setCity({
            code: cityCode
          }, cityIndex);
        ...
        if (isBelongToCity(countyCode, cityCode) && this.level >= 2 && countyIndex > -1) {
          this.setCounty({
            code: countyCode
          }, countyIndex);
        }
  ...
  var __vue_render__ = function() {
    ...
    return _c(
      "div",
      { staticClass: "el-select-area" },
      _vm._l(_vm.displayColumns, function(item, index) {
        return _c(
          "el-select",
          {
            key: index,
            ...
            attrs: {
              placeholder: _vm.placeholders[index],
              size: _vm.size,
              disabled: _vm.disabled
            },
            on: {
              change: function($event) {
                return _vm.handleOptionChange($event, _vm.values[index].type)
              }
            },
            model: {
              value: _vm.indexs[index],
              callback: function($$v) {
                _vm.$set(_vm.indexs, index, $$v);
              },
              expression: "indexs[index]"
            }
          },
          _vm._l(item, function(val, key) {
            return _c("el-option", {
              key: key,
              attrs: { label: val.name, value: key }
            })
          }),
          1
        )
  ...
  var areaList = list; // It's possible to expose named exports when writing components that can
